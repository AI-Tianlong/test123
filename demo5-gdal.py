...
def get_image(image_path):
    img_lists = find_data_list(image_path, suffix='tif')
    mat = []
    for idx in trange(len(img_lists),desc='正在将图像全部读入内存ing...',colour='GREEN'):
        # frame_copy = cv2.imread(img_lists[idx])  #这一步骤就是把图全读进来了，别这样把
        indataset = gdal.Open(img_lists[idx], gdal.GA_ReadOnly)#.ReadAsArray()
        indataset = indataset.ReadAsArray()
